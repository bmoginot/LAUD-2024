...
def run_kraken(pat):
    ...
    paths = sorted(glob.glob(f'/home/bmoginot/LAUD/patdata/{pat}/*')) # this folder contains fastq files for a given particpant pulled from EEK
    # iterate through sorted paths
    for i in range(0, len(paths), 2):
        # get paths to paired reads
        l_sample = paths[i] # forward read
        r_sample = paths[i+1] # reverse read
        # get sample number to name outfiles
        sig = l_sample.split('/')[-1][:4] # get the sample number (first field of fastq filename)
        # create paths for kraken2 output
        kraken_path = f'/home/bmoginot/LAUD/new_kraken/k2om_kfiles/{pat}/{sig}.out' # output folders are arbitrary; I named each file based on the sample number
        kreport_path = f'/home/bmoginot/LAUD/new_kraken/k2om_kreports/{pat}/{sig}.report' # kraken output two types of files, but we're only interested in the report files
        # get database path
        db = '/home/bmoginot/LAUD/krakendb'
        # run kraken per k2om specificiations
        os.system(f'kraken2 --db {db} --paired {l_sample} {r_sample} --threads 8 --use-names --report {kreport_path} --report-zero-counts --output {kraken_path}')
    return

def run_bracken(pat):
    # make pat directories
    if not os.path.isdir(f'/home/bmoginot/new_kraken/bfiles/{pat}'):
        os.mkdir(f'/home/bmoginot/new_kraken/bfiles/{pat}')
        os.mkdir(f'/home/bmoginot/new_kraken/breports/{pat}')
    # get paths to kraken reports
    paths = glob.glob(f'/home/bmoginot/new_kraken/k2om_kreports/{pat}/*')
    for report in paths:
        # get sample number
        sig = report.split('/')[-1][:4]
        # get database path
        db = '/home/bmoginot/krakendb'
        # create paths for bracken output
        bracken_path=f'/home/bmoginot/new_kraken/bfiles/{pat}/{sig}_new' # once again, output files are arbitrary
        breport_path=f'/home/bmoginot/new_kraken/breports/{pat}/{sig}_new' # once again, we're really only interested in the report files
        # run bracken
        os.system(f'bracken -d {db} -i {report} -r 100 -l S -t 10 -o {bracken_path}.bracken -w {breport_path}.breport')

def get_top_species(s):
    ...
    for sample in s:
        file = sample # read each bracken report one at a time
        ...
        # strip whitespace and add to list of species
        species_list.append([s.strip() for s in names])
    return species_list

def make_dict(samp, spec):
    ...
    for i in range(len(spec)-1):
        ...
        for j in range(i+1, len(spec)):
            samples = [samp[i], samp[j]]
            listy = [s for s in current if s in spec[j]]
...
def write_out(p, r):
    path = f'/home/bmoginot/new_kraken/{p}.txt'
    # write species and sample number and accompanying medium and location
    with open(path, 'w') as f:
        for k, v in r.items():
            f.write(str(k) + ': ' + str(v) + '\n')

def parse_bracken(pat):
    # get the path to each sample
    samples = glob.glob(f'/home/bmoginot/new_kraken/breports/{pat}/*') # gets all BRACKEN REPORTS for given pat --> should be the folder where you stored the breports when running bracken
    # make dictionary to get medium and isolation location for samples --> based on EEK data
    ...
    species_list = get_top_species(samples)
    # get list of sample names
    samples_list = [l.split('/')[-1][:4] for l in samples]
    ...
    for species in recurrent.keys():
        # crazy list comprehension (does what the block of comments above describes)
        recurrent[species] = [[num, reference_dict['PST'][int(num)], reference_dict['SL'][int(num)]] for num in recurrent[species]]
    # write out to text file
    write_out(pat,recurrent)

'''
Wrapper for running Kraken2 and Bracken
1. Build database to compare reads (ONLY DO THIS ONCE)
2. Generate kracken reports from fastq files
3. Generate bracken reports from kraken reports
4. Find recurrent species across samples
5. Output what species occur, which samples they occur in, and relevant EEK metadata associated with the sample
'''

def main():
    # build database ***YOU ONLY NEED TO DO THIS ONCE***
    os.system('kraken2-build --db krakendb --download-library bacteria') # reference library from NCBI against which reads are compared for kraken and bracken analysis
    # select desired participant data
    pat = 'pat1'
    # run kracken to generate reports
    run_kraken(pat)
    # run bracken on kraken reports
    run_bracken(pat)
    # manipulate bracken output
    parse_bracken(pat)
    return
